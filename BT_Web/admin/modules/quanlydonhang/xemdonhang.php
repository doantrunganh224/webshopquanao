 ...
 <?php
    $code=$_GET['code'];
    $sql_lietke_dh="SELECT * FROM tbl_cart_detail ,tbl_ao  WHERE tbl_cart_detail.id_sanpham=tbl_ao.id_ao 
...
<p>Liệt kê danh mục sản phẩm</p>
 <table style="width: 100%;" border="1" style="border-collapse:collapse;"> 
    ...
    while($row=mysqli_fetch_array($result_lietke_dh)){
        $i++;
        $thanhtien= $row['giaao'] * $row['soluongmua'];
        $tongtien+=$thanhtien;
     ?>
     <tr>
         <td><?php echo $i ?></td>
         ...
         <td><?php echo number_format($row['giaao'],0,',','.').'VNĐ' ?></td>
